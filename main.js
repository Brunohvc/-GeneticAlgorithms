var weightlimit = 30; // weight limit
...
var items = [];
items.push(new Item("Saco de dormir", 15.00, 15.00));
items.push(new Item("Corda", 7.00, 3.00));
items.push(new Item("Canivete", 10.00, 2.00));
items.push(new Item("Tocha", 5.00, 5.00));
items.push(new Item("Garrafa", 8.00, 9.00));
items.push(new Item("Comida", 17.00, 20.00));
...
Gene.prototype.encode = function (phenotype) {
    ...
    console.log(phenotype, this.genotype)
}
*/
...
Gene.prototype.onePointCrossOver = function (crossOverPr, anotherGene) {
    ...
    console.log("Coss: ", anotherGene)
    console.log(prob, crossOverPr)
...
var Population = function (size) {
    ...
    var gene1 = new Gene();
    gene1.genotype = [1, 0, 0, 1, 1, 0];
    gene1.calcFitness();
    var gene2 = new Gene();
    gene2.genotype = [0, 0, 1, 1, 1, 0];
    gene2.calcFitness();
    var gene3 = new Gene();
    gene3.genotype = [0, 1, 0, 1, 0, 0];
    gene3.calcFitness();
    var gene4 = new Gene();
    gene4.genotype = [0, 1, 1, 0, 0, 1];
    gene4.calcFitness();


    this.genes.push(gene1);
    this.genes.push(gene2);
    this.genes.push(gene3);
    this.genes.push(gene4);
...
Population.prototype.generate = async function () {
    // select the parents
    parents = this.select();
...
var population = new Population(4);
...
function display() {
    ...
    fitness.innerHTML = '1 - Survival Points:' + population.genes[0].fitness;
    fitness.innerHTML += '<br/>Genotype:' + population.genes[0].genotype;
    fitness.innerHTML += '<br/>Weight:' + population.genes[0].totalWeight;
    fitness.innerHTML += '<br/>Generation:' + population.genes[0].generation;

    fitness.innerHTML += '<br/><br/>2 - Survival Points:' + population.genes[1].fitness;
    fitness.innerHTML += '<br/>Genotype:' + population.genes[1].genotype;
    fitness.innerHTML += '<br/>Weight:' + population.genes[1].totalWeight;
    fitness.innerHTML += '<br/>Generation:' + population.genes[1].generation;

    fitness.innerHTML += '<br/><br/>3 - Survival Points:' + population.genes[2].fitness;
    fitness.innerHTML += '<br/>Genotype:' + population.genes[2].genotype;
    fitness.innerHTML += '<br/>Weight:' + population.genes[2].totalWeight;
    fitness.innerHTML += '<br/>Generation:' + population.genes[2].generation;

    fitness.innerHTML += '<br/><br/>4 - Survival Points:' + population.genes[3].fitness;
    fitness.innerHTML += '<br/>Genotype:' + population.genes[3].genotype;
    fitness.innerHTML += '<br/>Weight:' + population.genes[3].totalWeight;
    fitness.innerHTML += '<br/>Generation:' + population.genes[3].generation;
    ...
    for (var i = 0; i < 4; i++) {
        ...
        var centerX = radius + (0 + 1) * 5 + i * 2 * radius;
        context.beginPath();
        context.arc(centerX, centerY, radius, 0, 2 * Math.PI, false);
        
        console.log(index, population.genes[index])
        ...
        context.fillText('Survival Points:' + population.genes[index].fitness, centerX, centerY);
        index++;
    }
}
